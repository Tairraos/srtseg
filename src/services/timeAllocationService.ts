...
export class TimeAllocationService {
  ...
  public allocateTime(
    ...
    // 验证总时长是否一致
    this.validateTotalDuration(wordEntries, totalDuration);
   ...
   * @returns 剩余时间（毫秒）
   */
  private calculateRemainingDuration(startTime: string, endTime: string): number {
  ...
  private validateTotalDuration(
    ...
    if (Math.abs(actualDuration - expectedDuration) > tolerance) {
      console.warn(
        `时间分配警告: 期望时长 ${expectedDuration}ms，实际时长 ${actualDuration}ms`
      );
    }
  }
